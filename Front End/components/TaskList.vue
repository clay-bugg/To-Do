  ...
  <div class="task-list container">
    ...
    <transition-group tag="ul" v-if="listStore.selectedListType === 'tasks'" name="fade" class="item-list">
      <li v-for="(task, index) in taskStore.tasks" :key="task.id" class="list-item-box container">
        <div class="list-item">
          <!--Checked Task-->
          <button v-if="task.checked === true" class="checkbox" @click="completeTask(task.id)" :class="{ 'checked': task.checked }"  id="tick-icon" title="Complete">
            <Icon name="qlementine-icons:check-tick-16" class="checkbox-icon" />
          </button>
          <!--Unchecked Task-->
          <button v-if="task.checked === false" class="checkbox" @click="completeTask(task.id)" :class="{ 'checked': task.checked }"  id="tick-icon" title="Complete">
          </button>
          <!--Task Input-->
          <input v-if="task.editing === true" v-model="task.name" @keyup.enter="editTask(task.id)" class="task-edit-input" v-focus/>
          <p v-else-if="task.editing === false" @dblclick="editTask(task.id)" class="task-name">{{ task.name }}</p>
        </div>
        <!--Buttons-->
        <div class="list-item-buttons">
          <button class="list-item-button edit-button" :class="{ 'checked': task.checked }" @click="editTask(task.id)" title="Edit">
            <Icon v-if="task.editing === true" name="charm:tick" id="tick-icon" />
            <Icon v-else="task.editing === false" name="clarity:edit-solid" class="button-icon" id="edit-icon" />
          </button>

          <button class="list-item-button delete-button" :class="{ 'checked': task.checked }"  @click="deleteTask(task.id)" id="bin-icon" title="Delete">
    ...
    <transition-group tag="ul" v-if="listStore.selectedListType === 'completed'" name="fade" class="item-list">
      <li v-for="(task, index) in taskStore.completedTasks" :key="task.id" :class="{ 'checked': task.checked }" class="list-item-box container">
        <div class="list-item">
          <!--Checked Task-->
          <button v-if="task.checked === true" class="checkbox-checked" @click="uncheckTask" id="tick-icon" title="Complete">
            <Icon name="qlementine-icons:check-tick-16" class="checkbox-icon" />
          </button>
          <!--Unchecked Task-->
          <button v-if="task.checked === false" class="checkbox-unchecked" @click="checkTask">
            <Icon />
          </button>
          {{ task.name }}
        </div>
